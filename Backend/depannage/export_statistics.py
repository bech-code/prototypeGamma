...
def export_statistics_excel(request):
    """Exporte les statistiques complètes au format Excel (admin seulement)"""
    ...
    active_users_30d = User.objects.filter(
        Q(client_profile__repair_requests__created_at__gte=now - datetime.timedelta(days=30)) |
        Q(technician_profile__repair_requests__created_at__gte=now - datetime.timedelta(days=30))
    ).distinct().count()
